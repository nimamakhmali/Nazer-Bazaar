...
export default function ProvinceComplaintsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="search"
              placeholder="جستجوی عنوان شکایت، فروشگاه..."
              value={search}
      ...
      <div className="flex flex-wrap gap-2">
        {[
          { label: "کل",       value: totalCount,              color: "bg-slate-100 text-slate-700" },
          { label: "در انتظار",value: Math.floor(totalCount * 0.3), color: "bg-blue-100 text-blue-700" },
          { label: "در بررسی", value: Math.floor(totalCount * 0.2), color: "bg-amber-100 text-amber-700" },
          { label: "بسته",     value: Math.floor(totalCount * 0.5), color: "bg-green-100 text-green-700" },
        ].map((item) => (
          <span
            key={item.label}
            className={cn(
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{
                    ...
                    {["عنوان شکایت","فروشگاه","قیمت گزارش شده","وضعیت","زمان","عملیات"].map((h) => (
                      <th
                        key={h}
                ...
                <tbody>
                  {items.map((c) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <Link href={`/province/complaints/${c.uuid}`}>
