// src/app/(dashboard)/chamber/stores/page.tsx
"use client";
import React, { useState, useEffect, useCallback } from "react";
import {
  BuildingStorefrontIcon, MagnifyingGlassIcon, EyeIcon,
...
export default function ChamberStoresPage() {
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select
            ...
            className="px-3 py-2.5 text-sm border border-slate-200 rounded-xl
                       ...
                       bg-white appearance-none min-w-[160px]"
          >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ backgroundColor: "#f8fafc", borderBottom: "1px solid #f1f5f9" }}>
                    {["فروشگاه","اتحادیه","مالک","پروانه","وضعیت","عملیات"].map((h) => (
                    ...
                    <tr key={s.id}
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">{s.city_name}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4 text-slate-600 text-xs">{s.union_name}</td>
                      <td className="px-5 py-4 text-slate-600 text-xs">{s.owner_name}</td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <span className={cn(
                          ...
                          STORE_STATUS_COLORS[s.status],
                        )}>
                          {STORE_STATUS_LABELS[s.status]}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <Link href={`/chamber/stores/${s.id}`}>
