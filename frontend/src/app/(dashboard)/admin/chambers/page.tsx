...
export default function AdminChambersPage() {
  ...
  useEffect(() => {
    apiClient.get(ENDPOINTS.GEOGRAPHY.CITIES, { params: { page_size: 500 } })
      .then((res) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit && selected) {
        await apiClient.patch(ENDPOINTS.ORGANIZATIONS.CHAMBER(selected.id), payload);
  ...
  const handleAssign = async () => {
    if (!selected || !managerId) { toast.error("شناسه مدیر الزامی است"); return; }
    setSaving(true);
    try {
      await apiClient.post(ENDPOINTS.ORGANIZATIONS.CHAMBER_ASSIGN(selected.id), {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: "ادمین" }, { label: "سازمان‌ها" }, { label: "اتاق‌های اصناف" }]}
        actions={
              ...
              >
                ...
                <div className="flex items-start gap-3 mb-4">
                  <div
                    ...
                    style={{ background: "linear-gradient(135deg,#2E6DB4,#1B3A6B)" }}
                  >
                ...
                <div className="flex gap-2 mt-auto pt-3 border-t border-slate-100">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => { setSelected(chamber); setManagerId(""); setShowAssign(true); }}
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setViewUnions(viewUnions === chamber.id ? null : chamber.id)}
        ...
        <div className="space-y-4">
          {selected && (
            <div className="p-3 bg-slate-50 rounded-xl">
              ...
              <p className="text-xs text-slate-400">{selected.city_name}</p>
            </div>
          )}
          <Input
            label="شناسه کاربر (chamber_manager)"
            placeholder="شناسه عددی کاربر"
            value={managerId}
            onChange={(e) => setManagerId(e.target.value)}
            required
            type="number"
            hint="شناسه کاربر با نقش مدیر اتاق اصناف"
          />
        </div>
      </Modal>
    </div>
...
function ChambersUnions({ chamberId }: { chamberId: number }) {
  const [unions, setUnions] = useState<{ id: number; name: string; is_active: boolean }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    apiClient
      .get(ENDPOINTS.ORGANIZATIONS.CHAMBER_UNIONS(chamberId))
      .then((res) => { const d = res.data?.data ?? res.data; setUnions(extractArray(d)); })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [chamberId]);

  if (loading) return <p className="text-xs text-slate-400 pt-3">در حال بارگذاری...</p>;
  if (unions.length === 0) return <p className="text-xs text-slate-400 pt-3">اتحادیه‌ای ثبت نشده</p>;
  ...
  return (
    <div className="mt-3 pt-3 border-t border-slate-100">
      ...
      <div className="flex flex-wrap gap-1.5">
        {unions.map((u) => (
          <span key={u.id} className="px-2 py-0.5 bg-primary-50 text-primary-700 rounded-lg text-xs font-medium border border-primary-100">
            {u.name}
          </span>
        ))}
      </div>
