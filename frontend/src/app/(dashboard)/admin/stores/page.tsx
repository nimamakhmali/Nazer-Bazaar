...
const STATUS_CONFIG: Record<string, { label: string; variant: "success"|"danger"|"warning"|"info"|"default"; icon: React.ReactNode }> = {
  pending:   { label: "در انتظار",  variant: "warning", icon: <ClockIcon className="h-4 w-4" /> },
  active:    { label: "فعال",       variant: "success", icon: <CheckCircleIcon className="h-4 w-4" /> },
  suspended: { label: "تعلیق",      variant: "danger",  icon: <XCircleIcon className="h-4 w-4" /> },
  rejected:  { label: "رد شده",    variant: "danger",  icon: <XCircleIcon className="h-4 w-4" /> },
  closed:    { label: "تعطیل",      variant: "default", icon: <XCircleIcon className="h-4 w-4" /> },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select
            ...
            className="px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-100 bg-white appearance-none min-w-[160px]"
          >
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => { setSelected(store); setShowDetail(true); }}
                          ...
                          {store.status === "pending" && (
                            <>
                              <button
                                onClick={() => { setSelected(store); setShowApprove(true); }}
                              ...
                              <button
                                onClick={() => { setSelected(store); setReason(""); setShowReject(true); }}
        ...
        {selected && (
          <div className="space-y-4">
            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
              ...
              <div>
                <h3 className="font-bold text-lg text-primary-700">{selected.name}</h3>
                <p className="text-sm text-slate-400">{selected.union_name}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: "موبایل مالک",    value: selected.owner_phone },
                { label: "شماره پروانه",   value: selected.license_number },
                { label: "شهر",            value: `${selected.city_name} — ${selected.province_name}` },
                { label: "تلفن",           value: selected.phone || "—" },
                { label: "وضعیت",          value: STATUS_CONFIG[selected.status]?.label },
              ].map(({ label, value }) => (
            ...
            {selected.status === "pending" && (
              <div className="flex gap-2 pt-2">
                <Button
                  ...
                  size="sm"
                  fullWidth
                  onClick={() => { setShowDetail(false); setShowApprove(true); }}
                >
                  تایید فروشگاه
                </Button>
                <Button
                  ...
                  size="sm"
                  fullWidth
                  onClick={() => { setShowDetail(false); setReason(""); setShowReject(true); }}
                >
                  رد فروشگاه
                </Button>
              </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setShowReject(false)}>انصراف</Button>
            <Button variant="danger" onClick={handleReject} isLoading={acting}>
              رد کردن
            </Button>
          </>
        }
        ...
        <div className="space-y-4">
          {selected && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-xl">
              ...
              <p className="text-xs text-red-500 mt-0.5">{selected.union_name}</p>
            </div>
          )}
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-100 focus:border-red-400 resize-none"
            />
          </div>
        </div>
      </Modal>
