...
export default function UnionOfficialPricingPage() {
  ...
  // ── load products + today prices ────────────────────────────────────────────
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const [meRes, productsRes, todayRes] = await Promise.all([
          apiClient.get(ENDPOINTS.AUTH.ME),
          apiClient.get(ENDPOINTS.PRODUCTS.LIST, { params: { page_size: 200 } }),
          apiClient.get(ENDPOINTS.PRICING.OFFICIAL_PRICES_TODAY),
        ]);

        // get union_id from me endpoint (if available)
        const meData = meRes.data?.data ?? meRes.data;
        // union_id might come from a separate endpoint; use placeholder
        setUnionId(meData?.union_id ?? 1);
        ...
        setRows(
          products.map((p) => {
            ...
            return {
              ...
              yesterdayPrice: null,      // loaded lazily if needed
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: "اتحادیه" }, { label: "قیمت‌گذاری" }, { label: "ثبت قیمت مصوب" }]}
        actions={
          <div className="flex gap-2">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setRows((prev) => prev.map((r) => ({ ...r, inputValue: "" })))}
            >
              پاک کردن همه
            </Button>
            <Button
      ...
      <Card padding="md">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-sm font-bold text-slate-700">پیشرفت قیمت‌گذاری امروز</p>
      ...
      {loading ? (
        ...
        <Card padding="lg">
          <p className="text-center text-slate-400">محصولی برای قیمت‌گذاری یافت نشد</p>
        </Card>
      ) : (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-card overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ backgroundColor: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                  {[
                    "محصول", "دسته‌بندی", "واحد",
                    "قیمت دیروز", "قیمت مصوب امروز",
                    "حداقل مجاز (۸۰٪)", "وضعیت",
                  ].map((h) => (
              ...
              <tbody>
                {rows.map((row) => {
                  ...
                  const minPrice  = row.inputValue && inputNum > 0 ? calcMin(inputNum) : null;
                  const status    = getPriceStatus(row.inputValue, row.yesterdayPrice);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        {row.isSaved ? (
                          <Badge variant="success" dot size="sm">ثبت شده</Badge>
                        ) : row.inputValue ? (
                          status === "high" ? (
          ...
          <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/50
            ...
            <p className="text-xs text-slate-500">
              {savedCount} محصول ثبت شده از {rows.length} محصول
            </p>
            <Button
              onClick={saveAll}
              isLoading={saving}
