...
export function ComplaintTracker({ initialIdentifier = "" }: ComplaintTrackerProps) {
  const router = useRouter();

  const [complaint, setComplaint] = useState<Complaint | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [searchCode, setSearchCode] = useState(initialIdentifier);

  useEffect(() => {
    if (initialIdentifier) {
      setSearchCode(initialIdentifier);
      fetchComplaint(initialIdentifier);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-slate-50 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-primary-800">رهگیری وضعیت شکایت</h1>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-6 mb-6">
          ...
          <p className="text-xs text-slate-400 mt-2">کد رهگیری ۸ رقمی خود را وارد کنید</p>
        </div>
        ...
        {error && !complaint && !loading && (
          <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-8 text-center animate-fade-in">
            ...
            <h3 className="text-lg font-bold text-red-800 mb-2">شکایتی یافت نشد</h3>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden animate-fade-in">
            <div className="bg-gradient-to-r from-primary-700 to-primary-600 px-6 py-5">
              <div className="flex items-center justify-between">
                ...
                <div className="text-5xl">{getStatusConfig(complaint.status).emoji}</div>
              </div>
            </div>

            <div className="p-6 space-y-5">
              <div>
                <h2 className="text-xl font-bold text-primary-800 mb-2">{complaint.title}</h2>
                <div
                  className={cn(
              ...
              {complaint.description && (
                <div className="pt-4 border-t border-slate-100">
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed bg-slate-50 p-4 rounded-xl whitespace-pre-line">
                    {complaint.description}
                  </p>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="flex-1 py-3 bg-primary-700 text-white rounded-xl font-bold 
                             text-sm hover:bg-primary-800 transition-colors shadow-sm hover:shadow"
                >
                  📋 کپی کد پیگیری
                </button>
